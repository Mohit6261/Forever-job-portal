...
const ViewApplications = () => {
  ...
  //func tofetch company job applications data
  ...
  //func to update job appl status 

  const changeJobAplicationStatus=async(id,status)=>{
    try{
          ...
          <tr key={index} className='text-gray-700'>
            ...
            <td className='py-2 px-4 border-b relative'>
              ...
              <div className='z-10 hidden absolute right-0 md:left-0 top-0 mt-2 w-32 bg-white border border-gray-200 rounded shadow group-hover:block'>
                <button onClick={()=>changeJobAplicationStatus(applicant._id,'Accepted')} className='block w-full text-left px-4 py-2 text-blue-500 hover:bg-gray-100'>Accept</button>
                <button onClick={()=>changeJobAplicationStatus(applicant._id,'Rejected')}  className='block w-full text-left px-4 py-2 text-red-500 hover:bg-gray-100'>Reject</button>
              </div>
            </div>
            :
